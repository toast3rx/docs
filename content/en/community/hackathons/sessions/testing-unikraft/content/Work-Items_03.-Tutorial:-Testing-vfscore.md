...
```
After you select vfscore in `make menuconfig` a new option should appear in the `Library Configuration` menu to
select `ramfs` filesystem:

![ramfs menu](/community/hackathons/sessions/testing-unikraft/images/menuconfig_ramfs_option.png)

Make sure you check it otherwise the `mount` syscall will fail.
